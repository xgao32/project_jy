...
def vcf_maker(bedDir, genome, geneRegion_file, avinput, input, outputDir, CLASS, SYSTEM):
    ...
    # step 2: record db_bed information, initiate result dict for step 3, create temp bed for step 3
    bed = {}
    result = {}
    temp_bed = {}
    ...
    abpath = sys.path[0]
    s_v4 = abpath + "/supporting_files/supporting_software/s_v4.out"
    genomefai = genome + ".fai"
    fw = open(outputDir + "vcf_bloodRecord/" + sample + "_vcf_bloodRecord.txt","a") 
    for c in temp_bed.keys():
        cmd = "{s_v4} {bam} {bedfile} {hg} {hgfai} {output} ".format(s_v4=s_v4, bam = input, bedfile = temp_bed[c], hg=genome, hgfai=genomefai, output=outputDir + sample + "_" + all_CLASS[c] + "_" + c + "_s_v4_bloodRecord") 
        if (not os.path.exists(outputDir + sample + "_" + all_CLASS[c] + "_" + c + "_s_v4_bloodRecord_tumor_bmd.vcfDedup_mapping.txt")):
            os.system(cmd) # run s_v4.out
        while(os.path.exists(outputDir + sample + "_" + all_CLASS[c] + "_" + c + "_s_v4_bloodRecord_tumor_bmd.vcfDedup_mapping.txt") == "False"):
            time.sleep(0.5)
        with open(outputDir + sample + "_" + all_CLASS[c] + "_" + c + "_s_v4_bloodRecord_tumor_bmd.vcfDedup_mapping.txt") as fs:
        ...
        os.remove(outputDir + sample +"_" + all_CLASS[c] + "_" + c + "_s_v4_bloodRecord_tumor_bmd.vcfDedup_mapping.txt")
    fw.close()
    '''
