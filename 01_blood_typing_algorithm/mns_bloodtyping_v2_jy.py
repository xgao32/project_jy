...
def vcf_maker(dbDir, bedDir, genome, geneRegion_file, vcf_file, bam_file, inputDir, outputDir, CLASS):
    ...
    #cmd = "perl {}soft/annotate_variation.pl --format vcf4 {} > {}annotated_vcf/{}_vcf.avinput".format(inputDir, vcf_file, inputDir, sample)
    #os.system(cmd)
    while(os.path.exists(inputDir + "annotated_vcf/" + sample + "_vcf.avinput") == "False"):
        time.sleep(0.5)
    
    # step 4: read avinput file and record gene mutation
    with open(inputDir + "annotated_vcf/" + sample +"_vcf.avinput") as fv, open(outputDir + sample + "_vcf_bloodRecord.txt","w") as fw:
        line = fv.readline()
        while(line): 
            s = line.split("\t")
    ...
    for c in temp_bed.keys():
        cmd = "{s_v4} {bam} {bedfile} {hg} {hgfai} {output}".format(s_v4=s_v4, bam = bam_file, bedfile = temp_bed[c], hg=genome, 
    ...
    # step 6: compare with reference databse and find phenotype NNNYYYNNN
...
def main(inputDir, outputDir, vcf_file, bam_file, CLASS, SYSTEM):
   ...
   # set dbDir:
    ...
    vcf_maker(dbDir, bedDir, genome, geneRegion_file, vcf_file, bam_file, inputDir, outputDir, CLASS)
