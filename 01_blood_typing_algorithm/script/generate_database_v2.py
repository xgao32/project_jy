...
""""
This script, generate_database_v2.py, aims to convert nucleotide changes to genomic changes for blood group systems. It takes reference databases of blood group variants and maps their cDNA-based mutations to genomic coordinates using exon position information and reference genomes (hg19 and hg38).

Here's a breakdown of the code:

1. Functions:

complement(sequence): Returns the reverse complement of a DNA sequence.
searchseq(chr, g_start, g_end): Extracts a sequence from a reference genome FASTA file (hg38.fa) given chromosome, start, and end positions using samtools faidx.
cds_genomeLocation(exonFile, hgFile, hg_version, outdir):
Reads an exon position file (exonFile) and creates a dictionary mapping each gene to its CDS locations on the genome (hgFile).
Extracts CDS sequences from the genome based on the dictionary.
Creates another dictionary (siteDict) mapping each cDNA site to its corresponding genomic location and nucleotide.
genomeLocation(databaseFile, hg19_siteDict, hg38_siteDict, outdir, chr):
Reads a reference database file (databaseFile) containing blood group variants.
For each variant, it extracts the nucleotide change information.
Uses the siteDict dictionaries (for both hg19 and hg38) to convert the cDNA-based changes to genomic coordinates (g. notation).
Handles different mutation types: SNP, deletion, insertion, deletion-insertion, duplication, and replacement.
Writes the converted information to an output file and logs errors to a separate file.
2. Main Execution:

Sets input and output directory paths.
Defines paths to exon position files, reference genomes, and the database file.
Calls cds_genomeLocation to generate siteDict dictionaries for both hg19 and hg38.
Calls genomeLocation three times, each for a different database file, to perform the conversion and write the output.
In summary, this script automates the process of mapping blood group variant information from cDNA to genomic coordinates, which is crucial for accurate variant interpretation and analysis in a genomic context.
"""
import os 
import re
from Bio import Seq
...
def searchseq(chr, g_start, g_end):
    cmd = "samtools faidx /root/projects/projects_jy/WGS_blood/03_mns_blood_typing/input/Genome_refseq/hg38.fa {chr}:{g_start}-{g_end} > temp.fa".format(chr = chr, g_start = g_start, g_end = g_end)
    os.system(cmd)
...
def main(hg38_exonFile, hg19_exonFile, hg38_genome, hg38_version, hg19_genome, hg19_version, databaseFile, outdir):
    ...
    #1
    databaseFile = inputdir + "db_input/039_CTL2_Typer_database_hg38-hg19_v1.0_jy.txt"
    print(databaseFile)
    genomeLocation(databaseFile, hg19_siteDict, hg38_siteDict, outdir,"chr19")
    #2
    databaseFile = inputdir + "db_input/041_MAM_Typer_database_hg38-hg19_v1.0_jy.txt"
    print(databaseFile)
    genomeLocation(databaseFile, hg19_siteDict, hg38_siteDict, outdir, "chr19")
    #3
    databaseFile = inputdir + "db_input/042_EMM_Typer_database_hg38-hg19_v1.0_jy.txt"
    print(databaseFile)
...
hg19_exonFile = inputdir + "MNS-RH_exon_position_hg19_jy.txt"
hg38_exonFile = inputdir + "MNS-RH_exon_position_hg38_jy.txt"
...
databaseFile = inputdir + "db_input/002_MNS_Typer_database_hg38-hg19_v2_jy.txt"
