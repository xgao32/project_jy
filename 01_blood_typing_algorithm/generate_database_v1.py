        ...
        for line in lines:
            ...
            for m in range(len(ntChange)):
                ...
                if ((left == -1) & (right == -1)):
                    ...
                    # 2. delins type
                    if ("delins" in infor):
                        result = re.search(r"(.*):c.(\d+)_(\d+)delins(\w+)",infor)
                        gene = result.group(1)
                        ...
                        if (end <= start + 15):
                            for i in range(start, end+1):
                                delseq = delseq + siteDict[gene]["c."+str(i)][1]

                    # 3. del type
                    elif ("del" in ntChange[m][left+1, right]):
                    # 4. ins type
                    elif ("ins" in ntChange[m][left+1, right]):
                    # 5. dup type
                    elif ("dup" in ntChange[m][left+1, right]):
                    # 6. replace type
